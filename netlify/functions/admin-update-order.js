exports.handler = async function(event) {
  ...
  function deliveryNoteEmail(p){
    const rows=p.items.map((i,idx)=>{
      const sub=[i.color,i.capacidad||i.capacity,i.chip,i.ram,i.model_code].filter(Boolean).join(' · ');
      const warranty=itemWarranty(i),img=clean(i.image_url||'');
      const note=clean(i.item_note||''),features=clean(i.features||'');
      return `<tr><td style="padding:16px 0;border-bottom:1px solid #ececf1"><table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr>${img?`<td width="92" valign="top" style="padding-right:16px"><img src="${esc(img)}" alt="${esc(itemName(i))}" width="80" height="80" style="display:block;width:80px;height:80px;object-fit:contain;border-radius:14px;background:#f7f7f8"></td>`:''}<td valign="top"><b style="font-size:16px">${idx+1}. ${esc(itemName(i))}</b>${sub?`<br><span style="color:#6e6e73">${esc(sub)}</span>`:''}<br><span>Condición: <b>${esc(itemCondition(i))}</b></span><br><span>Número de serie: <b>${esc(itemSerial(i))}</b></span>${warranty?`<br><span>Garantía: <b>${warranty} días</b></span>`:''}${features?`<br><span style="color:#6e6e73">Características: ${esc(features)}</span>`:''}${note?`<br><span style="color:#6e6e73">Nota: ${esc(note)}</span>`:''}</td><td align="right" valign="top" style="white-space:nowrap;font-weight:700">${itemQty(i)} × ${money(itemPrice(i))}</td></tr></table></td></tr>`
    }).join('')||'<tr><td>Producto por confirmar</td></tr>';
    const content=`<div style="margin-bottom:22px"><div style="font-size:19px;font-weight:700;margin-bottom:8px">Gracias por tu compra, ${esc(p.customerName)}.</div><div style="color:#6e6e73;line-height:1.6">Tu compra fue registrada correctamente. Conserva esta nota como respaldo y consulta el seguimiento cuando lo necesites.</div></div><div style="border:1px solid #e3e3ea;border-radius:20px;padding:20px;margin-bottom:18px"><table width="100%" cellpadding="4" style="font-size:15px"><tr><td><b>Nota / pedido</b></td><td align="right">${esc(p.code)}</td></tr><tr><td><b>Cliente</b></td><td align="right">${esc(p.customerName)}</td></tr><tr><td><b>Documento</b></td><td align="right">${esc(p.customerDocument||'No indicado')}</td></tr><tr><td><b>Correo</b></td><td align="right">${esc(p.customerEmail)}</td></tr><tr><td><b>Teléfono</b></td><td align="right">${esc(p.customerPhone)}</td></tr><tr><td><b>Pago</b></td><td align="right">${esc(p.paymentMethod||'Por confirmar')}</td></tr><tr><td><b>Referencia</b></td><td align="right">${esc(p.paymentRef||'No aplica')}</td></tr><tr><td><b>Estado</b></td><td align="right">${esc(p.status)}</td></tr></table></div><div style="border:1px solid #e3e3ea;border-radius:20px;padding:20px;margin-bottom:18px"><h3 style="margin-top:0">Tu compra</h3><table width="100%" cellpadding="0" cellspacing="0">${rows}</table><div style="text-align:right;font-size:20px;font-weight:800;margin-top:18px">Total: ${p.total>0?money(p.total):'A confirmar'}</div></div><div style="padding:16px 18px;border-radius:16px;background:#f5f5f7;font-size:13px;line-height:1.6;color:#6e6e73"><b style="color:#1d1d1f">Política de garantía</b><br>La garantía cubre fallas de funcionamiento atribuibles al equipo durante el plazo indicado para cada producto. No cubre daños por golpes, humedad, manipulación externa, accesorios de terceros o intervenciones no autorizadas. La evaluación técnica determina la procedencia de la garantía.</div>`;
