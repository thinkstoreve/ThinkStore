  ...
  async function tsRefreshSelectedVariantAvailability(p){
    ...
    try{
      ...
      if(inv){
        ...
        else {label='Agotado';hint=reserved>0?'Existencia reservada en otros pedidos':'Sin existencias disponibles';key='out'}
      }
      ...
      if(quick){
        quick.innerHTML=`<b>Disponibilidad</b><small>${inv ? (available>0 ? `En stock · ${available} disponible${available===1?'':'s'}` : 'Agotado') : 'Bajo pedido · 15 a 25 días hábiles'}</small>`;
      }
      if(addBtn && inv){
        addBtn.disabled=available<1;
        addBtn.style.opacity=available<1?'.5':'';
        addBtn.style.cursor=available<1?'not-allowed':'';
        addBtn.textContent=available<1?'Agotado':'Añadir al carrito';
      }
    }catch(e){
