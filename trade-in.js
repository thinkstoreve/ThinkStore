(function(){
  ...
  function showQuote(){state.quote=estimate();$('quotePrice').textContent=`$${state.quote.low} – $${state.quote.high} USD`;const qs=$('quoteStatus');if(state.condition==='Con daños'){qs.textContent='Diagnóstico técnico obligatorio: el valor final se confirma únicamente después de la revisión presencial por Soporte Think.';qs.classList.add('diagnostic');}else{qs.textContent='Cotización preliminar válida por 7 días.';qs.classList.remove('diagnostic');}$('quoteBonus').textContent=state.accessories==='Con accesorios originales'?'Bono por accesorios originales completos aplicado.':'La estimación puede mejorar si presentas accesorios originales completos.';$('quotePrecheck').classList.add('hidden');$('preQuoteActions').classList.add('hidden');$('quoteResult').hidden=false;$('resultIntro').textContent='Esta es tu estimación preliminar ThinkStore.';try{localStorage.setItem('ts_last_tradein_quote',JSON.stringify({...state,createdAt:new Date().toISOString(),validUntil:new Date(Date.now()+7*86400000).toISOString()}));}catch(e){}}
